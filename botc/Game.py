...
with open('botc/game_text.json') as json_file:
    ...
    sv_lore = strings["gameplay"]["sv_lore"]
...
class Game(GameMeta):
    ...
    def __init__(self):
        ...
        self.nominator_list = [] # List of player objects
        self.alive_demon_and_voted_while_alive = self.alive_demon_and_false() # Dictionary of player object: Boolean
    ...
    @gamemode.setter
    def gamemode(self, new_gamemode):
        self._gamemode = new_gamemode
    ...
    def alive_demon_and_false(self):
        alive_demon_and_false_dict = {}
        for player in self.list_apparently_alive_players:
            if player.role.true_self.is_demon():
                alive_demon_and_false_dict[player] = False
        return alive_demon_and_false_dict
    ...
    async def send_lobby_welcome_message(self):
        ...
        gamemode_text = fancy.bold(self.gamemode.value)
        gamemode_botc_text = f"{gamemode_text} - 𝕭𝖑𝖔𝖔𝖉 𝖔𝖓 𝖙𝖍𝖊 𝕮𝖑𝖔𝖈𝖐𝖙𝖔𝖜𝖊𝖗 (𝕭𝖔𝕿𝕮)"
        pings = " ".join([player.user.mention for player in self.sitting_order])
        msg = lobby_game_start.format(pings, gamemode_text, self.nb_players)
    ...
    async def start_game(self):
        ...
        if self.gamemode == Gamemode.trouble_brewing:
            ...
            globvars.client.load_extension("botc.commands.abilities.sv")
    ...
    @property
    def nb_dead_players(self):
        """Return the number of dead players (apparently dead state)"""
        return len(self.list_apparently_dead_players)

    @property
    def list_apparently_alive_players(self):
        """Return the list of alive players (apparently alive state)"""
        return [player for player in self.sitting_order if player.is_apparently_alive()]
    ...
    @property
    def list_apparently_dead_players(self):
        """Return the list of dead players (apparently dead state)"""
        return [player for player in self.sitting_order if player.is_apparently_dead()]

    @property
    def list_dead_players(self):
        """Return the list of dead players (truly dead state)"""
        return [player for player in self.sitting_order if player.is_dead()]
    ...
    async def end_game(self):
        ...
        if self.gamemode == Gamemode.trouble_brewing:
            ...
            globvars.client.unload_extension("botc.commands.abilities.sv")
    ...
    async def make_dawn(self):
        ...
        # Initialize the temporary dawn data
    ...
    def generate_role_set(self):
        ...
        # Correct number of players and available BoTC edition
        elif self.gamemode in [Gamemode.trouble_brewing, Gamemode.bad_moon_rising, Gamemode.sects_and_violets]:
            ...
            if self.gamemode == Gamemode.trouble_brewing:
                townsfolk_all = BOTCUtils.get_role_list(TroubleBrewing, Townsfolk)
                outsider_all = BOTCUtils.get_role_list(TroubleBrewing, Outsider)
                minion_all = BOTCUtils.get_role_list(TroubleBrewing, Minion)
                demon_all = BOTCUtils.get_role_list(TroubleBrewing, Demon)

            # Bad moon rising mode
            elif self.gamemode == Gamemode.bad_moon_rising:
                townsfolk_all = BOTCUtils.get_role_list(BadMoonRising, Townsfolk)
                outsider_all = BOTCUtils.get_role_list(BadMoonRising, Outsider)
                minion_all = BOTCUtils.get_role_list(BadMoonRising, Minion)
                demon_all = BOTCUtils.get_role_list(BadMoonRising, Demon)

            # Sects and violets mode
            elif self.gamemode == Gamemode.sects_and_violets:
                from botc.gamemodes.sectsandviolets._utils import SnVRole
                townsfolk_all = BOTCUtils.get_role_list(SectsAndViolets, Townsfolk)
                townsfolk_all = [choice for choice in townsfolk_all
                                if choice.name not in [SnVRole.artist.value, SnVRole.savant.value]] # To be removed if these roles are implemented
                outsider_all = BOTCUtils.get_role_list(SectsAndViolets, Outsider)
                minion_all = BOTCUtils.get_role_list(SectsAndViolets, Minion)
                demon_all = BOTCUtils.get_role_list(SectsAndViolets, Demon)
