...
from botc import check_if_is_player, check_if_is_night, check_if_dm, RoleCannotUseCommand, \
    check_if_player_really_alive, check_if_can_dream, PlayerParser, AbilityForbidden, \
...
class Dream(commands.Cog, name = documentation["misc"]["abilities_cog"]):
    """BoTC in-game commands cog
    Dream command - used by dreamer
    """
    ...
    # ---------- DREAM COMMAND (Dreamer) ----------------------------------------
    @commands.command(
        pass_context = True,
        name = "dream",
        hidden = False,
        brief = documentation["doc"]["dream"]["brief"],
        help = documentation["doc"]["dream"]["help"],
        description = documentation["doc"]["dream"]["description"]
    )
    ...
    @commands.check(check_if_can_dream)  # Correct character -> RoleCannotUseCommand
    async def dream(self, ctx, *, dreamed: PlayerParser()):
        """Dream command
        usage: dream <player> and <player> and...
        characters: dreamer
        """
        player = BOTCUtils.get_player_from_id(ctx.author.id)
        await player.role.ego_self.register_dream(player, dreamed)

    @dream.error
    async def dream_error(self, ctx, error):
